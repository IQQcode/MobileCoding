...
//        mPaint.typeface = Typeface.createFromAsset(context.assets, "Muyao-Softbrush.ttf")//设置文字字体
    ...
    private fun measuredHeight(heightMeasureSpec: Int): Int {
        ...
        if (specMode == View.MeasureSpec.EXACTLY) {
            ...
            result =
                ((mPaint.fontMetrics.bottom - mPaint.fontMetrics.top) * mTextLine + getBaseline(
                    mPaint)).toInt()
    ...
    fun calcStrlineCount() {
        ...
        mTextLine =
            if (mPaint.measureText(mText) / width > (mPaint.measureText(mText) / width).toInt()) {//计算出文字的行数（如果文字的宽度除以控件的宽度是整数的话，就说明文字行数为文字除以控件宽度的值，反之则文字的行数还得再加1）
            ...
            for (i in 1..mTextLine) {//遍历行数（行数从1开始）
                if (mAnimProgress > mTextLineCountList[i - 1]) {
                    canvas!!.drawText(mText,
                        mTextLineCountList[i - 1],
                        mAnimProgress,
                        0f,
                        (mPaint.fontMetrics.bottom - mPaint.fontMetrics.top) * i,
                        mPaint)
                }
            }
        }
